...
import com.example.cb_dhs.screens.patientDetails.RiskAssessmentItem
...
class ReferralListViewModel(application: Application, private val fhirEngine: FhirEngine) :
    AndroidViewModel(application) {

    val liveSearchedReferrals = MutableLiveData<List<ReferralItem>>()
...
//    val liveServiceRequests: LiveData<MutableList<ServiceRequestPatient>> get() = _liveServiceRequests
    ...
    private fun updatePatientListAndPatientCount(
        search: suspend () -> List<ReferralItem>,
        count: suspend () -> Long,
    ) {
        viewModelScope.launch {
            liveSearchedReferrals.value = search()
    ...
    private suspend fun getSearchResults(nameQuery: String = ""): List<ReferralItem> {
        val patients: MutableList<ReferralItem> = mutableListOf()
        fhirEngine
            .search<Patient> {
                if (nameQuery.isNotEmpty()) {
                    filter(
            ...
            .mapIndexed { index, fhirPatient -> fhirPatient.resource.toReferralItem(index + 1) }
    ...
    /** The Referral's details for display purposes. */
    ...
    class ReferralListViewModelFactory(
...
internal fun Patient.toReferralItem(position: Int): ReferralListViewModel.ReferralItem {
    ...
    return ReferralListViewModel.ReferralItem(
